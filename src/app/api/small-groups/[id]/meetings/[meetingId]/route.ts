...
export async function GET(
  ...
  try {
    ...
    const formattedMeeting = {
      ...meeting,
      startTime: meeting.date.toISOString().split('T')[1].substring(0, 5), // Extrair HH:MM do datetime
      date: meeting.date.toISOString().split('T')[0], // Extrair apenas a data YYYY-MM-DD
      endTime: meeting.endTime ? meeting.endTime.toISOString().split('T')[1].substring(0, 5) : null
    };
